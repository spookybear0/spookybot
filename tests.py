...
class TestRecent(unittest.TestCase):
    def test_if_fail(self):
        try:
            r = asyncio.run(recent({}, ["!recent", "WhiteCat"]))
        except Exception as e:
            self.fail(e)
...
class TestTop(unittest.TestCase):
    def test_if_fail(self):
        try:
            r = asyncio.run(top({}, ["!top", "1", "WhiteCat"]))
        except Exception as e:
            self.fail(e)
